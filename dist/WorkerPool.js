...
var PoolWorker = function () {
  ...
  _createClass(PoolWorker, [{
    ...
    key: 'sendEOFMessage',
    value: function sendEOFMessage(err) {
      ...
      this.readBuffer(4, function (lengthReadError, lengthBuffer) {
        if (lengthReadError) {
          ...
          if (lengthReadError.name === 'EarlyEOFError') {
            _this.sendEOFMessage(lengthReadError);
          }
          return;
        }
        ...
        _this.readBuffer(length, function (messageError, messageBuffer) {
          if (messageError) {
            ...
            if (messageError.name === 'EarlyEOFError') {
              _this.sendEOFMessage(messageError);
            }
            return;
          }
    ...
    value: function onWorkerMessage(message, finalCallback) {
      ...
      switch (type) {
        case 'end':
          {
            var err = new Error('worker terminated');
            var _iteratorNormalCompletion = true;
            var _didIteratorError = false;
            var _iteratorError = undefined;

            try {
              for (var _iterator = Object.keys(this.jobs)[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
                var key = _step.value;

                var job = this.jobs[key];
                var jobCallback = job.callback;
  ...
  _createClass(WorkerPool, [{
    ...
    value: function disposeWorkers() {
      if (this.activeJobs === 0) {
        ...
        try {
          for (var _iterator3 = this.workers[Symbol.iterator](), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
